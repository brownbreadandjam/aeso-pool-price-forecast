...
# Our goal is to train, test, predict pool prices and forecast future values for the next hour, next day, and next week through machine learning methods. 
# We believe such predictions will have significance in forecasting Alberta pool prices and understanding patterns for traders and pool participants. 
...
# Checking mean, median, price range, and observations 
mean(aeso_data$Price) ; median(aeso_data$Price) ; range(aeso_data$Price, na.rm = TRUE)
quantile(aeso_data$Price, probs = c(0.05, 0.5, 0.95, 0.99), na.rm = TRUE)

# Huge differences in mean-median and price bounds with 95% observations concentrated within $407
# So we should setup a price cap and adjust for the difference in values  
...
# Signs of seasonality in hourly/weekly/monthly patterns evident
# This gave us a better understanding of which hours to lag such as 1 hour, 24 hour, 168 hour....
...
# Without spike detection features, models' prices flatline in test and forecast periods
# This is because model fails to distinguish gradual change from sudden shifts 
...
prediction_horizon <- 24 * 30
split_cutoff <- max(aeso_data$Datetime) - hours(prediction_horizon)
...
# This method of backcasting looped a stream of feedbacks during our iterations to design, modify, and omit features.
...
# Selectively omitted features from feedback of iterations 
...
# We tried different parameters and manually iterated inputs for better accuracy.
# We came to realize that XGBoost is able to handle all the lags and features we created, resulting in low error margins.
...
# This comparison plot shows how the model tracks the real prices closely, with some underestimation at extreme/unexpected spikes
# Despite the slight misalignment, the predicted prices outline the actual shaded area quite well, meaning the test accuracy of 77.5% is justified.
...
# Despite the accuracy, the residuals have some noise due to errors, but they are mostly bounded within 150, whereas price cap is $999
# Thus, we can infer that the model has considerably low error margins to design a realistic forecasting of next 7 days' pool prices.
...
# Previewing next-hour and next-day pool prices
print(head(forecast_table_7d, 24))

# Results for the 24 hour forecast are well aligned with existing patterns 
# We can infer that our model is in a working condition
# But we must check for errrors from the overall trend of last week prices vs next week forecasting




# Plotting the forecast with Actual vs XGboost prices
plot_data <- rbind(
...
# Since, the model is predicting values close to usual prices, we have scope to iterate and improve.
# This gives us a wide range of opportunity to try out different sorts of parameter tuning or ensemble with other models.
