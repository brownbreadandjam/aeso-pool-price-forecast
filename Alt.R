...
#Set directory: setwd("C:/Users/naved/Downloads")
...
ggplot(aeso_data, aes(price)) + geom_histogram(bins = 100, fill="darkgreen")
...
combined_features <- c(
  ...
  "ail_lag_1", "ail_lag_24", "roll_mean_24", "roll_sd_24", "spike_count_24","hrs_since_spike")
...
forecast_horizon_hours <- 24 * 30 *3
...
# p-value seems to be smaller 0.01, meaning data is stationary and may not need differencing.
...
arimax_pred <- back_transform(as.numeric(forecast(arimax_model, xreg = as.matrix(test_dt[, ..xreg_features]))$mean))
...
xgb_params <- list(
  objective = "reg:squarederror", eta = 0.03, max_depth = 3,
...
xgb_model <- xgb.train(
  params = xgb_params,  data = train_matrix, nrounds = 1000, early_stopping_rounds = 50,
  watchlist = list(train = train_matrix, test = test_matrix), verbose = 0)


# XGB Train & Test predictions have the lowest errors
xgb_model
...
# Creating a function to calculate root mean squared error and mean absolute error
...
# Creating results table to compare performance of the models
results <- rbind(
...
metric_fn <- function(actual, predicted, model_name, train_actual, train_predicted) {
  ...
  naive_mae <- mean(abs(diff(train_actual[idx_tr])))   # naive benchmark for MASE
  ...
  data.table(
    ...
    MASE       = round(test_mae / naive_mae, 3),
...
results <- rbind(
  metric_fn(test_dt$price, linear_pred,  "Linear",  train_dt$price, train_linear_pred),
  metric_fn(test_dt$price, arima_pred,   "ARIMA",   train_dt$price, train_arima_pred),
  metric_fn(test_dt$price, arimax_pred,  "ARIMAX",  train_dt$price, train_arimax_pred),
  metric_fn(test_dt$price, xgb_pred,     "XGBoost", train_dt$price, train_xgb_pred))
